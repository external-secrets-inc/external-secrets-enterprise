...
func (s *PostgresTestSuite) TestGenerateWithIdleCleanup() {
	...
	// Mock applyCronJob
	realApply := applyCronJob
	defer func() { applyCronJob = realApply }()
	called := false
	applyCronJob = func(ctx context.Context, c client.Client, manifest string, ownerRef metav1.OwnerReference) error {
		called = true
		return nil
	}
	...
	assert.True(s.T(), called, "applyCronJob should have been called")
