...
// Package workflow implements WorkflowRunTemplate handler.
package workflow
...
// Handler handles WorkflowRunTemplate secret rotation.
type Handler struct {
	ctx              context.Context
...
// Filter filters WorkflowRunTemplates based on the destination configuration.
...
// Apply applies the secret rotation to a WorkflowRunTemplate.
...
// WaitFor waits for the WorkflowRunTemplate to be ready.
...
// References checks if the WorkflowRunTemplate references the given secret.
...
func (h *Handler) _references(_ client.Object, _ string) (bool, error) {
	return true, nil
}
